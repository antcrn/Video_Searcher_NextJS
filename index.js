...
const reloadArticles = () => {
    ...
    articlesToShow.forEach((article) => {
        article.tags.forEach((tag) => {
            if (tag.includes(currentKeywords)) {
                document.querySelector('.articlesList').innerHTML += `
            <article>
                <h2>${article.titre}</h2>
            </article>
...
const cleanedKeyword = (keyword) => {
    ...
    return cleanedKeyword.toLowerCase();
};
    ...
    if (value.length >= 3) {
        ...
        keywordsCategories.forEach(category => {
            ...
            category.keywords.forEach(keyword => {
                if (keyword.substring(0, ).includes(value)) {
                    // console.log(category.name);
                    // console.log(keyword.substring(0, 3));
                    found = category.keywords;
                }
            })
        });
        ...
        found.forEach(key => {
            keyWordUl.innerHTML += `<li onclick="addNewKeyword('${key}','${ cleanedKeyword(key)}')">${key}</li>`;
